...
const validationSchema = Yup.object().shape({
    ...
    sources: Yup.string().max(1000).required('Nhập link phim')
});
...
function FormUpdate() {
    ...
    const [dateRelease, setDateRelease] = useState(undefined);
    ...
    const handleSubmitForm = (values) => {
        ...
        payload.append('DateRelease', dateRelease?.value ?? convertTime(movieInfo?.body?.dateRelease));
        ...
        return dispatch(Movie.CreateOrUpdate(payload, idMovie));
    }
    useEffect(() => {
        dispatch(Movie.GetInfo(idMovie));
    }, [dispatch, idMovie]);

    const movieUpdate = useSelector(state => state.movieUpdate);
    return (
    <React.Fragment>
        {(movieInfo?.loading || movieUpdate?.loading) && <Loader />}
        <MainCard title={`Cập nhật phim ${movieInfo?.body?.title ?? ""}`}>
            <Breadcrumbs 
                titleBottom={true}
                title={true} 
                rightAlign={true} 
                ...
                <CardContent>
                    {movieInfo?.body &&
                     <Box component="div">
                        <Formik
                            initialValues={{
                                title: movieInfo?.body?.title ?? "",
                                description: movieInfo?.body?.description ?? "",
                                sources: movieInfo?.body?.source ?? "",
                                backDrop: movieInfo?.body?.backDrop ?? "",
                                poster: movieInfo?.body?.poster ?? ""
                            }}
                                                ...
                                                <FormControl sx={{ width: '100%', marginTop: '10px' }}>
                                                    <LocalizationProvider dateAdapter={AdapterDateFns}>
                                                        <DesktopDatePicker
                                                            ...
                                                            value={dateRelease?.label ?? movieInfo?.body?.dateRelease}
