...
function GetInfo(genreId){
    return async dispatch => {
        ...
        const url = `/api/v1/genre/${genreId}`;
        const response = await instanceRequest(url, axiosMethods.GET);
        try {
            if(response.success) {
...
function CreateOrUpdate(payload, genreId = "") {
    return async dispatch => {
        const request = genreId ? constants.UPDATE_REQUEST : constants.CREATE_REQUEST;
        await dispatch({ type: request });
        let url = `/api/v1/genre`;
        if(genreId) url += `/${genreId}`; 
        const response = await instanceRequest(url, genreId ? axiosMethods.PUT : axiosMethods.POST, payload);

        try {
            if(response.success) {
                toast.success(response.message);
                return await dispatch({ type: genreId ? constants.UPDATE_SUCCESS : constants.CREATE_SUCCESS, payload: response.data });
            }
            else {
                await dispatch({ type: genreId ? constants.UPDATE_FAILURE : constants.CREATE_FAILURE, payload: response.message });
            ...
            const message = "Internal Server Error";
            toast.error(message);
            return await dispatch({ type: genreId ? constants.UPDATE_FAILURE : constants.CREATE_FAILURE, payload: message });
        }
    }
}
...
function Delete(genreId){
    return async dispatch => {
        ...
        try {
            if(response.success) {
                toast.success(response.message);
                return await dispatch({ type: constants.DELETE_SUCCESS, payload: genreId });
            }
            else {
